...
export default {
  ...
  methods: {
    getCas() {
      this.$axios
          .get("http://localhost:8080/workload/getCas")
          .then((res) => {
    ...
    getPodLog() {
      ...
      this.$axios
          .get("http://localhost:8080/workload/getPodLog", {
            params: {
              podNamespace: podn,
    ...
    handleDelete(index, row) {
      this.$confirm(`您确定删除吗?`, '提示', {
        ...
        this.$axios
            .delete("http://localhost:8080/workload/deletePodLog/" + row.id).then((response) => {
