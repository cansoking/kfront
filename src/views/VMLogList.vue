<template>
  <div class="vmarea">
    ...
    <el-table :data="vmlogdata.slice((curpage - 1) * pagesize, curpage * pagesize)" style="width: 100%" empty-text="暂无日志"
      ...
      <el-table-column fixed="right" label="操作">
        <template slot-scope="scope">
          <el-button size="mini" type="success" @click="lookLog(scope.$index, scope.row)">查看详细日志</el-button>
    ...
    <div v-if="vmlogdata.length != 0" style="margin-top: 30px">
      <el-pagination :current-page.sync="curpage" :page-sizes="[10, 20, 30, 40, 50]" :page-size.sync="pagesize"
        layout="sizes, total, prev, pager, next, jumper" :total="totalvmlog" background></el-pagination>
    </div>
...
export default {
  ...
  methods: {
    getSaveDays() {
      this.$axios
          .get(this.baseurl + "/log/getVMSaveDays")
          .then((res) => {
