        ...
        <template slot-scope="props">
          <el-form label-position="left" inline class="demo-table-expand">
            ...
            <el-form-item label="创建时间">
              ...
              <div>
                <span>{{
                  formatDate(props.row.metadata.creationTimestamp, "YYYY-MM-DD")
                }}</span><br />
                <span>{{
...
export default {
  ...
  methods: {
    ...
    startPod(idx, row) {
      this.$axios({
        method: "post",
        url: this.baseurl + "/workload_k8s/startPod",
        data: {
    ...
    stopPod(idx, row) {
      this.$axios({
        method: "post",
        url: this.baseurl + "/workload_k8s/stopPod",
        data: {
          podName: row.metadata.name,
          podNamespace: row.metadata.namespace,
          podNodeName: row.spec.nodename,
    ...
    cp_sumbit(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.$axios({
            method: "post",
            url: this.baseurl + "/workload_k8s/createPod",
            data: {
              // podName: this.cp_form.podName,
              // podNamespace: this.cp_form.namespace,
              // podNodeName: this.cp_form.nodename,
    ...
    deletePod(idx, row) {
      this.$confirm('此操作将永久删除该Pod, 是否继续?', '提示', {
        ...
        this.$axios({
          method: "post",
          url: this.baseurl + "/workload_k8s/deletePod",
          data: {
            podName: row.metadata.name,
            podNamespace: row.metadata.namespace,
            podNodeName: row.spec.nodename,
    ...
    migrate_sumbit(idx, row, formName) {
      ...
      this.$refs[formName].validate((valid) => {
        ...
        if (valid) {
          this.$axios({
            method: "post",
            url: this.baseurl + "/workload_k8s/editPod",
            data: {
              podName: row.metadata.name,
              podNamespace: row.metadata.namespace,
    ...
    nsremote() {
      this.$axios
        .get(this.baseurl + "/workload_k8s/namespace/list")
        .then((res) => {
          // console.log(res);
    ...
    noderemote() {
      this.$axios
        .get(this.baseurl + "/workload_k8s/node/list")
        .then((res) => {
          // console.log(res);
          console.log(JSON.parse(res.data.result));
          let rdata = JSON.parse(res.data.result).items;
